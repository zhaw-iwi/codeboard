...
var createLtiUser = function (req) {
  let lis_person_name_given = req.body.lis_person_name_given || '';
  lis_person_name_given = lis_person_name_given.replace(/[^A-Za-zàâçéèêÀÂÇÉÈÊ]/g, '');
...
var submitGradeToLtiToolConsumer = function (projectId, ltiSessionId, ltiUser, ltiNonce, grade) {
  ...
  return db.Project.findByPk(projectId)
    ...
    .then(function (ltiSession) {
      ...
      if (ltiSession === null) {
        const error = new Error(
          'No valid LTI session for {ltiSession: ' +
            ltiSessionId +
            ', ltiUser: ' +
            ltiUser +
            ', ltiNonce: ' +
    ...
    .then(function (res) {
      ...
      if (res.statusCode !== 200) {
        const error = new Error(
          'Error while submitting grade to LTI TC platform. Try to submit again or report the problem to the course admin'
        );
        return Promise.reject(error);
      }
...
var isValidLtiUserSession = function (ltiSessionId, ltiUserId, ltiNonce) {
  return db.LtiSession.findOne({
    ...
    .catch(function (err) {
      console.error(
        'DB error: checking valid LtiSession for {ltiSessionId: ' +
          ltiSessionId +
          ', litUserId: ' +
          ltiUserId +
          ', ltiNonce: ' +
 ...
 * @return {*} a promise that resolves to true if a session with the given parameters exists, otherwise false
 */
var isValidLtiUserProjectSession = function (projectId, ltiSessionId, ltiUserId, ltiNonce, courseId = null) {
  return db.LtiSession.findOne({
    ...
    .then(function (session) {
      if (session !== null) {
        // check if the courseId is matching with the courseId set in the ltiSession
        // This should always be the case, because even if no course was set, the values should match. So both NULL
        if (session.CourseId !== courseId) {
          const error = new Error(
            "Access rejected because user's supplied courseId (" +
              courseId +
              ') does not match with the courseId stored in the ltiSession (' +
              session.CourseId +
              ')'
          );
          error.status = 401;
