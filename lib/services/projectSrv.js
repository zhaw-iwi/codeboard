 ...
 * @param scope the scope for which to check a resource (project / course)
 * @returns {*|Promise} resolved to true or false
 */
const isValidProjectId = function (projectId) {
  return db.Project.findByPk(projectId).then(function (prj) {
    return prj !== null;
  });
};
 ...
 * @returns {*|Promise} resolved to true or false
 */
const isPublicProject = function (projectId) {
  return db.Project.findByPk(projectId).then(function (prj) {
    var result = false;
 ...
 */
const getProjectOwners = function (projectId) {
 ...
 * @returns a promise that resovles to an array of usernames
 */
const getProjectUsers = function (projectId) {
  return db.Project.findOne({
    where: { id: projectId },
    include: [{ model: db.User, as: 'userSet', attributes: ['username'] }],
  }).then(function (prj) {
    var result = [];
 ...
 */
const isProjectOwner = function (projectId, username) {
  return getProjectOwners(projectId).then(function (ownerArray) {
 ...
 */
const isProjectUser = function (projectId, username) {
  return getProjectUsers(projectId).then(function (userArray) {
 ...
 */
const isProjectOwnerOrUser = function (projectId, username) {
  var checks = [];
  ...
  return Promise.some(checks, 2).then(function (result) {
...
/**
 * Defines the relationship between courses and projects
 *
 ...
 * @param listOfCourses
 */
const setCoursesOfProject = function (project, listOfCourses) {
  return db.Course.findAll({
    where: { id: { [Op.in]: listOfCourses } },
  }).then(function (courses) {
    return project.setCourseSet(courses).then(function () {
      return project;
    });
  });
};

const setUsersOfProject = function (project, listOfUserNames, asOwners) {
  return db.User.findAll({ where: { username: { [Op.in]: listOfUserNames } } })
    .then(function (users) {
      if (users === null) {
        return project;
      }
 ...
 */
const setUsersAsOwners = function (project, listOfUserNames) {
 ...
 */
const setUsersAsUsers = function (project, listOfUserNames) {
 ...
 */
const addNewFiles = function (fileArray, project) {
 ...
 */
const updateExistingFiles = function (fileArray, project) {
  /* the promises is an array of promises */
  var promises = [];
  ...
  for (var j = 0; j < fileArray.length; j++) {
    var p = new Promise(function (resolve, reject) {
      db.File.findByPk(fileArray[j].id)
        .then(function (file) {
          ...
          file.isStatic = updateValues[file.id].isStatic;
 ...
 */
const removeFilesFromProject = function (fileIdsToDelete, project) {
  // delete all the files that the user deleted
  return db.File.destroy({ where: { id: fileIdsToDelete } });
};

/**
 * Gets the project description and sample solution for a project (used for AI prompts).
 * @param projectId the project id for which to retrieve the files
 * @returns {*|Promise}
 */
const getProjectFiles = async function (projectId) {
  ...
  const result = {
    ...
    solution: 'No sample solution available',
    staticFiles: 'No static files available',
  };

  // get project description and sample solution
  if (project.fileSet && project.fileSet.length > 0) {
    project.fileSet.forEach((file) => {
      if (file.filename === 'projectDescription.html') {
        // remove HTML tags
        result.description = file.content.replace(/(<([^>]+)>)/gi, '');
      } else if (file.filename === 'sampleSolution.html') {
        // remove HTML tags
        result.solution = file.content.replace(/(<([^>]+)>)/gi, '');
      }
    });
  }

  // get static files
  const staticFiles = project.fileSet.filter((file) => file.isStatic);
  if (staticFiles.length > 0) {
    staticFiles.forEach((file) => {
      result.staticFiles += file.content + '\n';
    });
  }
 ...
 */
const getAllHiddenFilesOfProject = function (projectId, includeHiddenFolders) {
  ...
  return db.Project.findAll(
    // note: need to use findAll again because of the 'where' in the include; it's a bug in sequelize
    {
      where: { id: projectId },
      include: [{ model: db.File, as: 'fileSet', where: queryConstraint }],
    }
  ).then(function (prj) {
...
const getAllStaticFilesOfProject = async function (projectId) {
  const staticFiles = await db.File.findAll({ where: { projectId: projectId, isStatic: 1 } });
  if (!staticFiles) {
    throw new Error('No static files found for the given project.');
  }
  return staticFiles;
};

/**
 * Returns the content of all changable user files in a project
 * @param {string} userId the id of the current logged in user
 * @param {number} courseId the id of the course for which to retrieve the files.
 * @param {number} projectId the id of the project for which to retrieve the files.
 * @returns {string} the content of the changable user files
 */
const getAllChangeableUserFiles = async function (userId, courseId, projectId) {
  const userCode = await db.UserProject.findAll({
    where: { courseId: courseId, projectId: projectId, userId: userId },
    attributes: ['id', 'files', 'updatedAt', 'projectId', 'courseId', 'projectId', 'userId'],
  });

  // check if userCode is empty
  if (!userCode) {
    throw new Error('No user code found for the given project and user.');
  }
  ...
  userCode.forEach((file) => {
    ...
    filteredUserCode = userFilesArray.filter((file) => !file.isStatic);
  });
 ...
 */
const getCombinedFiles = function (hiddenFiles, userFiles) {
 ...
 */
const removeTestResult = function (input) {
  var _result = input;
 ...
 */
const extractTestResult = function (input) {
 ...
 */
const getShortProjectDescription = function (fullProjectDescription, shortLength) {
...
/**
 * Returns a submission if the requested user has successfully submitted his code.
 * @param prjId the id of the project
 * @param usrId the id of the requested user
 * @param crsId the id of the course
 * @return {String} the shortened description
 */
const checkSubmission = async function (prjId, usrId, crsId) {
  const hasSubmission = await db.Submission.findOne({
    where: { projectId: prjId, userId: usrId, testResult: 1, courseId: crsId < 0 ? null : crsId },
  });
 ...
 */
const recalculateUniqueIds = function (_fileArray) {
  ...
  for (var i = 0; i < fileArray.length; i++) {
    ...
    if (fileArray[i].uniqueId !== 0) {
      // don't set the parentUId for the root folder (root folder has no empty path and parentUId of -1)
...
module.exports = {
  ...
  getProjectFiles,
  getAllHiddenFilesOfProject,
  getAllStaticFilesOfProject,
  getAllChangeableUserFiles,
  removeTestResult,
  extractTestResult,
  getCombinedFiles,
  getShortProjectDescription,
  checkSubmission,
  recalculateUniqueIds,
};
