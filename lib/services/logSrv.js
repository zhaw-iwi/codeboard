...
var showCompilationPerDayLogs = function (startDate, endDate, projectId, res) {
  ...
  compilerModel.aggregate( [ filterCompiler, { $group: {
      ...
      .exec( function (err, compilerLogs) {
        if(err) {
          console.log("Error DB"); res.send(404);
        }

        result.compilerLogs = compilerLogs; // store 'compile' logs
        compilerModel.aggregate( [ filterRun, { $group: {
            _id: { year: { $year: "$startTime" }, month: { $month: "$startTime" }, day: { $dayOfMonth: "$startTime" } },
            totalTime: { $sum: "$time" },
            averageTime: { $avg: "$time" },
            min: {$min: "$time" },
            ...
            .exec( function (err, compilerLogs) {
              ...
              compilerModel.aggregate( [ filterTest, { $group: {
                  _id: { year: { $year: "$startTime" }, month: { $month: "$startTime" }, day: { $dayOfMonth: "$startTime" } },
                  totalTime: { $sum: "$time" },
                  averageTime: { $avg: "$time" },
                  min: {$min: "$time" },
                  ...
                  .exec( function (err, compilerLogs) {
                    ...
                    compilerModel.aggregate( [ filterTool, { $group: {
                        _id: { year: { $year: "$startTime" }, month: { $month: "$startTime" }, day: { $dayOfMonth: "$startTime" } },
                        totalTime: { $sum: "$time" },
                        averageTime: { $avg: "$time" },
                        min: {$min: "$time" },
      ...
      .exec( function (err, compilerLogs) {
        ...
        compilerModel.aggregate( [
          ...
          { $group: {
            ...
            .exec( function (err, compilerRunLogs) {
              if(err) {
                  console.log("Error DB"); res.send(404);
              }
...
var showProjectAccessPerUser = function (res) {
  ...
  pageViewModel.aggregate( [
    ...
    { $group: {
      ...
      .exec( function (err, projectAccess) {
        if(err) {
          console.log("Error DB"); res.send(404);
        }
...
var showActiveProjectPerDayLogs = function (startDate, endDate, res) {
  ...
  pageViewModel.aggregate( [
    { $match: filter },
    { $group: { _id: { year: { $year: "$date" }, month: { $month: "$date" }, day: { $dayOfMonth: "$date" }, projectId: "$projectId" } } },
...
var showCompilationsPerLanguage = function (startDate, endDate, res) {
  ...
  compilerModel.aggregate( [
    ...
    { $group: {
      ...
      .exec( function (err, compLanguages) {
          if(err) {
              console.log("Error DB"); res.send(404);
          }
        ...
        compilerModel.aggregate( [
          ...
          { $group: {
            ...
            .exec( function (err, compOnlyLanguages) {
                if(err) {
                    console.log("Error DB"); res.send(404);
                }
              ...
              compilerModel.aggregate( [
                ...
                { $group: {
                  ...
                  .exec( function (err, runOnlyLanguages) {
                    if(err) {
                      console.log("Error DB"); res.send(404);
                    }
...
var showCompilationsErrors = function (startDate, endDate, res) {
  ...
  compilerModel.aggregate( [
    ...
    { $group: {
      ...
      .exec( function (err, normalCompLanguages) {
        if(err) {
          console.log("Error DB"); res.send(404);
        }
        ...
        compilerModel.aggregate( [
          ...
          { $group: {
            ...
            .exec( function (err, slowCompLanguages) {
              if(err) {
                  console.log("Error DB"); res.send(404);
              }
              ...
              compilerModel.aggregate( [
                ...
                { $group: {
                  ...
                  .exec( function (err, tooSlowCompLanguages) {
                    if(err) {
                      console.log("Error DB"); res.send(404);
                    }
                    ...
                    compilerModel.aggregate( [
                      ...
                      { $group: {
                        ...
                        .exec( function (err, errorCompLanguages) {
                          if(err) {
                              console.log("Error DB"); res.send(404);
                          }
...
var showPopularProjects = function (startDate, endDate, limit, res) {
    ...
    pageViewModel.aggregate([
        ...
        .exec(function (err, popularProjects) {
            if (!err) {
