 ...
 * This service is used for logs and project statistics (actions on project, accesses and submissions). It uses mongoDB.
 */
...
// Connect to database -> already done in server.js
...
const compilerModel = mongoose.model('compilerlog', compilerSchema); // todo angepasst
...
const pageViewModel = mongoose.model('pageviewlog', pageViewSchema);
var events = {};
...
var addActionLog = function (req, startTime, data, error) {
  ...
  log
    ...
    .then((res) => {})
    .catch((err) => {
      console.log('Error in the log: ' + err);
    });
};
...
var showCompilationPerDayLogs = function (startDate, endDate, projectId, res) {
  ...
  if (projectId == undefined) {
    filterCompiler = {
      $match: { $and: [{ action: 'compile' }, { endTime: { $gte: inputStartDate, $lte: inputEndDate } }] },
    };
    filterRun = {
      $match: {
        $and: [
...
var showProjectAccessPerUser = function (res) {
  ...
  pageViewModel
    .aggregate([
      { $match: { $or: [{ pageCode: '11' }, { pageCode: '9' }] } },
      {
        $group: {
          _id: '$userName',
          lastDate: { $last: '$date' },
...
var showSubmitPerProject = function (projectId, startDate, endDate, res) {
  ...
  pageViewModel
    .aggregate([
      {
        $match: {
          $and: [{ projectId: projectId }, { pageCode: '16' }, { date: { $gte: inputStartDate, $lte: inputEndDate } }],
        },
      },
      {
        $group: {
...
var showProjectAccessPerDayLogs = function (startDate, endDate, projectId, res) {
  ...
  if (projectId == undefined) {
    filter = {
      $and: [{ $or: [{ pageCode: '11' }, { pageCode: '9' }] }, { date: { $gte: inputStartDate, $lte: inputEndDate } }],
    };
  } else {
    filter = {
      $and: [
...
var showActiveProjectPerDayLogs = function (startDate, endDate, res) {
  ...
  pageViewModel
    .aggregate([
      ...
      { $group: { _id: { year: '$_id.year', month: '$_id.month', day: '$_id.day' }, count: { $sum: 1 } } },
    ])
    .then(function (accesses) {
...
var showCompilationsPerLanguage = function (startDate, endDate, res) {
  ...
  compilerModel
    ...
    .then(function (compLanguages) {
      result.languageLogs = compLanguages; // calculate compilations only per language
      compilerModel
        .aggregate([
          { $match: { $and: [{ action: 'compile' }, { startTime: { $gte: inputStartDate, $lte: inputEndDate } }] } },
          {
            $group: {
              _id: '$language',
              count: { $sum: 1 },
        ...
        .then(function (compOnlyLanguages) {
          result.languageCompilationLogs = compOnlyLanguages; // calculate runs only per language
          compilerModel
            .aggregate([
              { $match: { $and: [{ action: 'run' }, { startTime: { $gte: inputStartDate, $lte: inputEndDate } }] } },
              {
                $group: {
                  _id: '$language',
                  count: { $sum: 1 },
            ...
            .then(function (runOnlyLanguages) {
...
var showCompilationsErrors = function (startDate, endDate, res) {
  ...
  compilerModel
    ...
    .then(function (normalCompLanguages) {
      result.normal = normalCompLanguages[0] !== undefined ? normalCompLanguages[0].count : 0;
      ...
      compilerModel
        ...
        .then(function (slowCompLanguages) {
          result.slow = slowCompLanguages[0] !== undefined ? slowCompLanguages[0].count : 0;
          ...
          compilerModel
            ...
            .then(function (tooSlowCompLanguages) {
              result.tooSlow = tooSlowCompLanguages[0] !== undefined ? tooSlowCompLanguages[0].count : 0;
              ...
              compilerModel
                ...
                .then(function (errorCompLanguages) {
                  result.error = errorCompLanguages[0] !== undefined ? errorCompLanguages[0].count : 0;
                  res.send(result);
                })
                .catch(function (err) {
                  console.log('Error DB');
...
events.failedLtiSigninEvent = function (req) {
  return {
    pageCode: 18, // 'failed lti authentication',
    date: new Date(),
    ltiUserId: req.body.user_id,
    description: 'LTI authentication failed',
  };
};

events.LtiSigninEvent = function (req, user) {
  return {
    pageCode: 19, // 'lti authentication',
    date: new Date(),
    ltiUserId: req.body.user_id,
  };
};
