...
var test = function (data, testSubmission) {
  ...
  return request.post;
  Async({ url: url, json: true, body: payload }).then(function (res, body) {
  ...
  for (var i = 0; i < fileData.length; i++) {
    if (fileData[i].filename.indexOf(coboConfigFileName) !== -1) {
      ...
      try {
        ...
        dirOfTestSubmissionFiles = config[dirOfTestSubmissionFilesProperty]
          ? config[dirOfTestSubmissionFilesProperty]
          : dirOfTestSubmissionFiles;
      } catch (e) {
