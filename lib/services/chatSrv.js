      ...
      return getChatLine(chatLine.id).then(function (chatLine) {
        ...
        if (aChatLine.authorId !== aChatLine.userId && chatLine.author.username !== 'Roby') {
          mailSrv
            ...
            .then(() => aChatLine)
            .catch((err) => {
              console.log('Error sending answer mail:', err);
              return aChatLine;
            });
        }
        // otherwise we send an email to the teacher with the information about the new help requests
        ...
        // is generated by the llm
          ...
          mailSrv
            ...
            .then(() => aChatLine)
            .catch((err) => {
              console.log('Error sending help request mail:', err);
              return aChatLine;
            });
        }
 ...
 * @param projectId
 */
const deleteChatLines = async function (chatIds, projectId, userId) {
  // fetch chat lines first to validate ownership
  const chatlines = await ChatLine.findAll({
    where: {
      id: chatIds,
      userId,
      projectId,
  ...
  if (chatlines.length !== chatIds.length) {
    throw new Error('One or more chatlines not found or access denied');
  }

  // delete the chat lines
  return await ChatLine.destroy({
    where: {
      id: chatIds,
      userId,
      projectId,
