 ...
 */
const getUser = function (username, attributes = ['id', 'username', 'role', 'email']) {
 ...
 */
const getUserById = function (userId, attributes = ['id', 'username', 'name', 'role', 'email']) {
 ...
 * Assumes: user
 */
const updateLtiUsers = function (username, new_pw, email) {
  // find and change users password
  return db.User.findOne({ where: { username: username }, attributes: ['id', 'password'] }).then(function (usr) {
    if (usr !== null) {
      usr.password = new_pw;
      usr.email = email;
