...
var postRequestHTTP = function (url, aPostData, aMantraNode) {
  ...
  return request
    .postAsync({ url: url, json: true, headers: headersContent, body: aPostData })
    .then(function (res, resBody) {
      resBody = res.body;

      // handle the error cases
      if (res.statusCode >= 400) {
        // create an error object
        const error = new Error(resBody.msg);
        ...
        if (resBody.stream == false) {
          ...
          if (resBody.hasOwnProperty('compilationError')) {
            ...
            successPayload.compilationOutputArray = resBody.compilationOutputArray;
          }
        } else {
          ...
          successPayload.stopUrl = '/api/stopContainer' + resBody.stop.pathname;
...
var startContainer = function (aUrl, aMantraId, aContainerId) {
  ...
  return request.postAsync({ url: mantraUrl, headers: lHeaders }).then(function (res) {
