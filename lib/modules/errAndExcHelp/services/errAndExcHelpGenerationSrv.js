 ...
 * @returns {boolean}
 */
const isRegexValid = function (regex) {
 ...
 * @returns {boolean}
 */
const isStringMatchingRegex = function (string, regex, maxStringLength = 2000) {
  ...
  if (string.length > maxStringLength) {
    console.log(
      `Output ${string.length} exceeds max output length of ${maxStringLength} and will therefore not be checked with regex`
    );
    return false;
  }
 ...
 * @returns {boolean}
 */
const findExceptionInOutput = function (exceptionSet, output) {
  let exception = false;
 ...
 */
const getRandomTextBlock = function (set, block_type = 'intro', exceptionId = null) {
 ...
 * @param langId
 */
const getErrAndExcExceptions = function (langId) {
 ...
 */
const getErrAndExcHelpSet = function (langId, courseId) {
 ...
 * @returns {*}
 */
const getPlaceholdersByArray = function (placeholderIds) {
 ...
 * @returns {*}
 */
const findAndReplaceDynamicPlaceholders = function (msg, output) {
  ...
  if (match) {
    ...
    if (indexes.length > 0) {
      return getPlaceholdersByArray(indexes).then(function (placeholders) {
        let id, ph, match;
 ...
 * @returns {*}
 */
const findAndReplaceStaticPlaceholders = function (msg, output, course, user) {
  let placeholders = {};
 ...
 * @returns {Promise}
 */
const createHelpMessageForException = function (exception, output, lang, course, user) {
  let courseId = course ? course.id : 0;
  return getErrAndExcHelpSet(lang.id, courseId).then(function (aHelpSet) {
    if (!aHelpSet) {
      const error = new Error(`No help set found for language ${lang.id} and course ${courseId}`);
    ...
    return Promise.all(promises).then(function (textBlocks) {
 ...
 * @returns {*}
 */
const getExceptionOnFirstLine = function (outputArray, lang) {
  return getErrAndExcExceptions(lang.id).then(function (errAndExcExceptionsSet) {
    let errorsOnFirstLine = [];
    outputArray.forEach(function (separatedOutput, index) {
      // exception.line & exception.output
 ...
 *  @param courseId
 */
const getHelpForCompilationErrors = function (output, language, userId, courseId) {
  if (!output) {
    const error = new Error('Output is missing');
    return Promise.reject(error);
  }
  if (!language) {
    const error = new Error('Language is missing');
    return Promise.reject(error);
  }
  ...
  return Promise.all([getLang, getCourse, getUser]).then(function ([lang, course, user]) {
    if (!lang) {
      const error = new Error('Language is required');
      return Promise.reject(error);
    }
    // find exception
    return getExceptionOnFirstLine(outputArray, lang).then(function ([exception, outputObject]) {
