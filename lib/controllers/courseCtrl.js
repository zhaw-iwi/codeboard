 ...
 * @param res
 */
let getCourseProjects = async function(req, res) {
    try {
      // to do: auth?
      ...
      const courseProjects = await db.Course.findOne({
        where: {id: courseId },
        include: [
          { model: db.Project, as: "projectSet", required: false }
        ]
      })
    ...
    helpRequestSrv.getAllHelpRequestsForCourse(courseId, _viewQuery, _userIdQuery)
        .then(function (data) {
            ...
            data.forEach(function (helprequest) {
                var promise = submissionSrv.getSubmissions(helprequest.projectId, helprequest.userId, courseId, orderBy)
