 ...
 * Assumes: the projectId in the route is valid
 */
var getLimitedProject = async function (req, res) {
  try {  
    // extract the id from the route
    var projectId = req.params.projectId;
 ...
 */
var getSampleSolution = async function(req, res) {
  try {
    // extract the id from the route
    var projectId = req.params.projectId;
      ...
      .then(function(course) {
        ...
        .catch(function(err) {
          ...
          if (err && err.statusCode === 401) {
            return res.status(401).json({msg: 'You are not authorized to access this page. Please check that this project allows for LTI access and that key/secret are correct.'});
          } else if (err && err.statusCode === 500) {
            return res.status(500).json({msg: 'Error while creating an LTI session. Please try again. If the error persists, please contact support.'});
          }
          return res.status(520).json({msg: 'An unknown error occurred. Please try again. If the error persists, please contact support.'});
        });
      });
    } else {
...
var createSubmission = function (req, res) {
  ...
  projectSrv.getAllHiddenFilesOfProject(projectId, false)
      ...
      .then(function (testResult) {
        ...
        return submissionSrv.getSubmissions(projectId, userId, courseId, orderBy)
...
let getAllHelpRequests = function(req, res) {
  ...
  helpRequestSrv.getAllHelpRequestsForProject(projectId, _viewQuery, _userIdQuery)
      .then(function (data) {
        let promises = [];
        var orderBy = [['testResult', 'DESC'], ['updatedAt', 'DESC']];
        data.forEach(function (helprequest) {
          var promise = submissionSrv.getSubmissions(projectId, helprequest.userId, null, orderBy)
              .then(function (submissions) {
