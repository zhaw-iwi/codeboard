...
/**
 * Because of the new moodle/lti version we can't define
 * access to the resource via the path in the route. Instead we need to 
 * define route parameters in the query string.
 */
const redirectLtiUrl = function (req, res) {
  const projectId = req.body.project_id; // lti sends the project id as 'id' in the body
  if (!projectId) {
    return res.status(400).send({msg: 'No project id provided.'});
  }
  res.redirect(307, `/lti/projects/${projectId}`);
}
