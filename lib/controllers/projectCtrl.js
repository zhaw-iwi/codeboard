 ...
 */
const isAuthorizedOwner = function (req, res) {
 ...
 */
const getProjectSettings = function (req, res) {
...
const getProjectSummary = function (req, res) {
  ...
  db.Project.findAll({
    ...
    where: { id: projectId },
    include: [{ model: db.User, as: 'ownerSet', attributes: ['username'] }],
  }).then(function (result) {
 ...
 */
const getFullProject = function (req, res) {
  // extract the id from the route
  var projectId = req.params.projectId;
  ...
  return db.Project.findOne({
    ...
    include: [
      {
        ...
        through: { attributes: [] },
        include: [{ model: db.CourseOption, as: 'courseOptions', attributes: ['option', 'value'] }],
      },
    ...
    .then(function (prj) {
      if (prj === null) res.status(404).json({ message: 'The project does not exist.' });
      else {
        /* logging */
 ...
 * Assumes: the projectId in the route is valid
 */
const getLimitedProject = async function (req, res) {
  try {
    // extract the id from the route
    var projectId = req.params.projectId;

    // extract the course id from the url parameters
    var courseId = req.query.courseId ? parseInt(req.query.courseId) : -1;

    // userId is used to check wheter sample solution can be sent to client
    var userId;
    ...
    // check if requested user already has a successful submission
    const hasSubmission = await projectSrv.checkSubmission(projectId, userId, courseId);
    ...
    // check if the requested user already has a successfull submission to add the sample solution to the response
    if (hasSubmission) {
      projectQuery.include.push({
        model: db.File,
        as: 'sampleSolution',
        where: { filename: 'sampleSolution.html' },
        required: false,
      });
    }

    const project = await db.Project.findByPk(projectId, projectQuery);
    ...
    if (project === null) {
      ...
      // set the role of the user
      project.dataValues.userRole = 'user';

      // we only want one course in a limited call
      if (req.query.courseId && project.courseSet.length > 0) {
        project.dataValues.course = project.courseSet.find((course) => course.id === courseId);
      }
      delete project.dataValues.courseSet; // we delete the courseSet in a limited call

      res.status(200).json(project);
    }
  } catch (err) {
    console.log(err);
    return res.status(500).json({ message: 'Error while fetching limited project data.' });
  }
};

/**
 * Returns only the sample solution after a successful submission.
 */
const getSampleSolution = async function (req, res) {
  try {
    // extract the id from the route
    var projectId = req.params.projectId;
    ...
    if (hasSubmission) {
      const result = await db.Project.findByPk(projectId, {
      ...
      // check if sample solution is available because not every project has a sample solution
      if (result.sampleSolution) {
        res.status(200).json(result.sampleSolution.dataValues.content);
      } else {
        res.status(404).send('No sample solution found!');
      }
    } else {
      res.status(404).send('No valid submission found.');
    }
  } catch (err) {
    console.log('Error while fetching sample solution: ' + err.message);
    return res.status(500).json({ message: 'Error while fetching sample solution.' });
  }
};
 ...
 */
const getUserProject = function (req, res) {
  ...
  return db.UserProject.findOne({
    ...
    if (userProject) {
      ...
      db.Submission.findOne({
        ...
        var userFiles = JSON.parse(userProject.files);
        // remove the sampleSolution from the files array > to do: redundant check because of existing users sample solution is still stored in userprojects table
        userFiles = userFiles.filter((file) => file.filename !== 'sampleSolution.html');
        ...
        res.status(200).json({
          ...
          lastSubmissionHasReview: !!(submission && submission.hasCodeReview),
        });
      });
    } else {
 ...
 */
const putProjectSettings = function (req, res) {
  ...
  db.Project.findByPk(projectId)
    ...
    .catch(function (err) {
      console.log(err);
      res.status(500).json({ message: 'Error while updating project settings' });
    });
};
 ...
 */
const createProjectForUser = function (req, res) {
  // create tmp project
  let tmpPrj = {
  ...
  return db.Project.create(tmpPrj)
    .then(function (project) {
      ...
      projectRelations.push(
        templateSrv.createProjectFromTemplate(project.language).then(function (newFiles) {
      ...
      // add owner set
      projectRelations.push(
        db.User.findOne({ where: { username: tmpPrj.username } }).then(function (user) {
    ...
    .catch(function (err) {
      console.log(err);
      res.status(500).send({ message: 'Error while create project for user.' });
    });
};
...
const putProject = function (req, res) {
  ...
  db.Project.findOne({
    where: { id: projectId },
    include: [{ model: db.File, as: 'fileSet', attributes: ['id'] }],
  })
    .then(function (prj) {
      ...
      var currentFileIds = [];
      for (var i in prj.fileSet) currentFileIds.push(prj.fileSet[i].dataValues.id);

      // get the file ids that are send with the request and have 'id > 0', i.e. they are an update of a file in the db
      var fileIdsInReq = [];
      for (var i in req.body.files) if (req.body.files[i].id > 0) fileIdsInReq.push(req.body.files[i].id);
    ...
    .then(function () {
      ...
      var filesToUpdate = [];
      for (var i in req.body.files) if (req.body.files[i].id > 0) filesToUpdate.push(req.body.files[i]);
    ...
    .then(function () {
      ...
      var newFiles = [];
      for (var i in req.body.files) if (req.body.files[i].id < 0) newFiles.push(req.body.files[i]);
...
/**
 * Save a a version of a project for a user must access project via a course (but not the owner of the project).
 * Assumes: a valid projectId
 * Assumes: the user is authenticated
 * Assumes: request body has lastUId info and files array
 */
const putProjectForUser = function (req, res) {
  // get the projectId
  var projectId = req.params.projectId;
...
const getUserProjects = function (req, res) {
  ...
  if (req.isAuthenticated() && requestedUser === req.user.username) {
    ...
    db.User.findOne({
      attributes: ['username', 'emailPublic', 'name', 'url', 'location', 'institution'],
      ...
      .catch(function (err) {
        console.log(err);
        res.status(500).json({ message: 'Error while fetching user projects.' });
      });
  } else {
 ...
 * @param res
 */
const _runActionOnProject = function (req, res) {
  ...
  // can we use this condition for compileandrun? Before the else condition was triggered
  // this is used for "Test (compile)" and "Run" (compileandrun) button
  if (action === 'compile' || action === 'compileandrun') {
    compilationSrv
      ...
      .catch(function (err) {
        console.log('Error in compile: ' + JSON.stringify(err));
 ...
 */
const runLimitedActionOnProject = function (req, res) {
  ...
  if (req.body.action !== 'run') {
    ...
    projectSrv.getAllHiddenFilesOfProject(projectId, false).then(function (hiddenFileSet) {
...
/**
 * Executes an action (e.g. 'compile', 'run') on a project (as an owner).
 * @param req request containing the files, the action etc.
 * @param res response that will be returned to the caller
 */
...
const startContainer = function startContainer(req, res, next) {
  mantraSrv
    .startContainer(req.url, req.params.mantraId, req.params.containerId)
    .then(function (result) {
...
const stopContainer = function stopContainer(req, res, next) {
  mantraSrv
    .stopContainer(req.url, req.params.mantraId, req.params.containerId)
    .then(function (result) {
      res.status(res.statusCode).json({ msg: res.msg });
    })
    .catch(function (err) {
 ...
 */
const initLtiSession = function (req, res) {
  // the id of project
    ...
    return db.Course.findOne({
      ...
      return db.LtiSession.create({
        ...
        .then(function (ltiSession) {
          ...
          const error = new Error('Error while creating an LTI session in the database.');
        ...
        .then(function (ltiSession) {
          ...
          for (var propertyInReqBody in req.body) {
            // remove the custom_project_id from the uri parameters as it is not needed in final uri
            if (propertyInReqBody.substring(0, 7) === 'custom_' && propertyInReqBody !== 'custom_project_id') {
              // get the parameter in from key=value (but remove the prefixed "custom_")
              var parameter = propertyInReqBody.substring(7) + '=' + req.body[propertyInReqBody];
          ...
          if (ltiSession.CourseId !== -1) {
            return res.redirect('/courses/' + ltiSession.CourseId + '/projects/' + _projectId + '?' + _uriParams);
          }
          return res.redirect('/projects/' + _projectId + '?' + _uriParams);
        })
        .catch(function (err) {
          ...
          if (err && err.statusCode === 401) {
            return res.status(401).json({
              msg: 'You are not authorized to access this page. Please check that this project allows for LTI access and that key/secret are correct.',
            });
          } else if (err && err.statusCode === 500) {
            return res.status(500).json({
              msg: 'Error while creating an LTI session. Please try again. If the error persists, please contact support.',
            });
          }
          return res.status(520).json({
            msg: 'An unknown error occurred. Please try again. If the error persists, please contact support.',
          });
        });
    });
  } else {
    return res.status(401).json({ msg: 'You are not authorized to access this course.' });
  }
};
 ...
 *          - The lti request is authorized
 */
const _initLtiSession = function (req, res) {
  // the id of project
  var _projectId = req.params.projectId;
  ...
  db.Project.findByPk(_projectId)
    .then(function (prj) {
      ...
      if (!prj.isLtiAllowed || !isRequestAuthorized(req, prj.ltiSecret)) {
        ...
        const error = new Error(
          'The project does not allow access via LTI or the provided key/secret combination is wrong.'
        );
    ...
    .then(function () {
      ...
      // return a redirect; this will trigger a 'get' request for the project with parameters about the user and lti session
      var _uriParams = encodeURI(
        'ltiSessionId=' + _ltiSession.id + '&ltiUserId=' + _userId + '&ltiNonce=' + req.body.oauth_nonce
      );
    ...
    .catch(function (err) {
      ...
      if (err && err.statusCode === 401) {
        ...
        res
          .status(520)
          .json({ msg: 'An unknown error occurred. Please try again. If the error persists, please contact support.' });
      }
    });
};
...
const createSubmission = function (req, res) {
  ...
  projectSrv
    ...
    .then(function (testResult) {
      ...
      var orderBy = [['testResult', 'DESC']];
      return submissionSrv.getSubmissions(projectId, userId, courseId, orderBy).then(function (submissions) {
        // submit if either no prev submissions or even current submission is better/equals to the prev best
        let submitToLti = submissions.length === 0 || submissions[0].testResult <= result;
        ...
        _submissionRecord.ltiSessionId = req.body.hasLtiData
          ? req.body.ltiData.ltiSessionId
          : _submissionRecord.ltiSessionId;
        ...
        // log the submission of the user
        logSrv.addPageLog(logSrv.events.submitEvent(req));

        // check if the requested user i an owner so we do not have to store a submission
        if (data.userRole === 'owner') {
          return res.status(200).json({
            msg: 'Your solution was successfully submitted but not stored in the db.',
            data: _submissionRecord,
          });
        }

        return db.Submission.create(_submissionRecord).then(function (submissionRecord) {
          ...
          if (submitToLti && submissionRecord.hasResult && submissionRecord.ltiSessionId !== -1) {
            ...
            ltiSrv
              .submitGradeToLtiToolConsumer(projectId, _ltiId, _ltiUserId, _ltiNonce, submissionRecord.testResult)
              .then(function (result) {
            ...
            res.status(200).json({
              msg: 'Your solution was successfully submitted.',
 ...
 */
const getAllSubmissions = function (req, res) {
  ...
  submissionSrv
    .getAllSubmissionsForProject(projectId, _viewQuery, _userIdQuery)
    .then(function (data) {
...
const getSubmission = function (req, res) {
 ...
 * @param res
 */
const createHelpRequest = function (req, res) {
  let projectId = req.params.projectId,
    data = req.body;
 ...
 * @param res
 */
const updateHelpRequest = function (req, res) {
 ...
 * @author Janick
 */
const getAllHelpRequests = function (req, res) {
  // get the projectId from the Url path
  var projectId = req.params.projectId;
  // there could be a query ?view=compact
  var _viewQuery = typeof req.query.view !== 'undefined' ? req.query.view : null;
  ...
  helpRequestSrv
    .getAllHelpRequestsForProject(projectId, _viewQuery, _userIdQuery)
    .then(function (data) {
      let promises = [];
      var orderBy = [
        ['testResult', 'DESC'],
      ...
      data.forEach(function (helprequest) {
        var promise = submissionSrv
          .getSubmissions(projectId, helprequest.userId, null, orderBy)
 ...
 * @param res
 */
const getHelpRequest = function (req, res) {
...
const getAllUserProjectsForProject = function (req, res) {
  var _projectId = req.params.projectId;
 ...
 * @param res
 */
const getUserProjectForProject = function (req, res) {
  ...
  db.UserProject.findOne({
    ...
    .then(function (aFullUserProject) {
      if (!aFullUserProject) {
        ...
        const error = new Error(
          'Error retrieving userProject with id ' + _userprojectId + ' for project with id ' + _projectId
        );
        error.statusCode = 500;
        return Promise.reject(error);
      } else {
 ...
 * @param res
 */
const getHelpForCompilation = function (req, res) {
  const projectId = req.params.projectId;
  const data = req.body;

  errAndExcHelpSrv
    .getHelpForCompilationErrors(data.compilation.output, data.language, req.user.id, data.courseId)
...
module.exports = {
  ...
  getAllHelpRequests,
  getHelpRequest,
  getAllUserProjectsForProject,
  getUserProjectForProject,
  getHelpForCompilation,
};
