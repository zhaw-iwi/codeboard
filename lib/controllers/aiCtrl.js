...
var getRelevantHint = async function (req, res) {
  try {
    ...
    const project = await db.Project.findOne({
      where: { id: _data.projectId },
      attributes: ["id"],
      include: [
        {
          model: db.File,
          as: "fileSet",
          where: { ProjectId: _data.projectId },
    ...
    const userCodeContent = await projectSrv.getAllChangeableUserFiles(_data.userId, _data.courseId, _data.projectId);
    ...
    let hints = JSON.stringify(_data.hints);
...
var getCompilerExplanation = async function (req, res) {
  try {
    ...
    const user = await userSrv.getUserById(_data.userId);
...
var getCodeReview = async function (req, res) {
  try {
    ...
    } else if (submission.testResult === 0 || submission.hasCodeReview) {
