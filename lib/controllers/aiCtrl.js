...
var getUsageStats = async function (req, res) {
  try {
    if (
      req.isAuthenticated() &&
      (req.user.username === "admin" || req.user.username === "javakurs")
    ) {
      var stats = await db.AIUsage.findAll({
        attributes: [
          "id",
          "promptTokens",
          "completionTokens",
      ...
      res.setHeader(
        "Content-disposition",
        "attachment; filename=usage-stats.csv"
      );
    ...
    res.status(500).json({
      message: "An error occurred while fetching the AI usage stats: " + err
    });
  }
};
