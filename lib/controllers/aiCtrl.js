...
const getHelp = async function (req, res) {
  try {
    ...
    // update the corresponding chatLine with the answer from the AI
    // to do: return the updated chatLine to the client
    const updatedChatLine = await chatSrv.updateChatLine(chatLineId, { message: help });
...
const getRelevantHint = async function (req, res) {
  try {
    ...
    const data = {
      userCodeContent: userCodeContent,
      projectFiles: projectFiles,
      hintHistory: [], // to do: get the previous hints from the database
      userId: userId,
      courseId: courseId,
      projectId: projectId,
    };

    var hint = await AiSrv.askForHint(data);
