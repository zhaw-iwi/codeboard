...
const requestSrv = require('./services/requestSrv.js');
...
var isAuthImp = function (req, res, next, onFailUseNextRoute) {
  if (req.isAuthenticated()) next();
    ...
    if (onFailUseNextRoute) next('route');
...
var isValidProjectId = function (req, res, next) {
  ...
  projectSrv.isValidProjectId(projectId).then(function (isValid) {
    if (isValid) {
...
/**
 * Route can only be accessed if the courseId in the route is a valid id.
 * Executes next() or sends a 404.
 *
 * @author Janick Michot
 */
var isValidCourseId = function (req, res, next) {
  return db.Course.findByPk(req.params.courseId).then(function (prj) {
    if (prj !== null) {
      next();
    } else {
...
/**
 * Implementation for the checking if the authenticated user is a admin.
 * Assumes: user is authenticated
 */
const isAdmin = function (req, res, next) {
  const admins = ['admin', 'javakurs'];
  if (admins.includes(req.user.username)) {
    return next();
  }
  return res.status(403).send({ statusCode: 403, msg: 'You are not allowed to access this resource.' });
};
...
var isSelf = function (req, res, next) {
  if (req.user.username === req.params.username) next();
  else res.sendStatus(401);
};
...
var isOwnerImpl = function (req, res, next, onFailUseNextRoute) {
  ...
  projectSrv
    .isProjectOwner(projectId, username)
...
var isUserImpl = function (req, res, next, onFailUseNextRoute) {
  ...
  projectSrv
    .isProjectUser(projectId, username)
...
var isOwnerOrUserImpl = function (req, res, next, onFailUseNextRoute) {
  ...
  projectSrv
    .isProjectOwnerOrUser(projectId, username)
    .then(function (isOwnerOrUser) {
      if (isOwnerOrUser) next();
      else {
        if (onFailUseNextRoute) next('route');
        else res.sendStatus(401);
      }
 ...
 */
const setProjectIdFromLtiRequest = function (req, res, next) {
  // if a custom project id was set in moodle (new version) we need to set the projectId as param
  if (req.body.custom_project_id) {
    req.params.projectId = req.body.custom_project_id;
    return next();
  }
  return res.status(400).send({ statusCode: 400, msg: 'No project id was set in the request.' });
};
...
var isLtiUserElseNextRoute = function (req, res, next) {
  ...
  if (!ltiData.hasLtiData) {
    ...
    ltiSrv
      .isValidLtiUserProjectSession(projectId, ltiData.ltiSessionId, ltiData.ltiUserId, ltiData.ltiNonce, courseId)
      .then(function (isValidLtiUserSession) {
        if (isValidLtiUserSession) {
          ...
          return Promise.reject(new Error("Access rejected because user's supplied LTI session data is invalid."));
        }
      })
      .catch(function (err) {
        ...
        if (req.method === 'GET') {
          ...
          console.log(
            'Method: ' + req.method + ',  OriginalURL: ' + req.originalUrl + ',  Data: ' + JSON.stringify(req.body)
          );
        }
        console.log(err);
        next('route');
...
var isLtiRequestAuthorized = function (req, res, next) {
  ltiSrv
    ...
    .catch(function (err) {
      console.log('Error while initializing an LTI session. Lti request not authorized.' + err.msg);
      res
        .status(500)
        .send({ statusCode: 500, msg: 'Error while initializing an LTI session. Lti request not authorized.' });
    });
};
...
let isCourseOwnerImpl = function (req, res, next, onFailUseNextRoute) {
  ...
  courseSrv
    .isCourseOwner(courseId, username)
    .then((isOwner) => {
      if (isOwner) {
        next();
      } else {
...
let isProjectInCourse = function (req, res, next) {
  courseSrv
    .isProjectInCourse(req.params.courseId, req.params.projectId)
    .then((isBelonging) => (isBelonging ? next() : res.sendStatus(401)))
    .catch((err) => res.sendStatus(500));
};

/**
 * If courseId as request parameter is undefined > define it as -1
 * Important for ai services
 * > if a user tries to request help using ai from a project not in a course return 404
 * > owner can also not use ai services (todo: implement functionality?)
 */
var hasCourseId = function (req, res, next) {
  if (req.params.courseId === 'undefined' || req.params.courseId === undefined) {
...
/**
 *
 * This function checks if the user has reached the request limit for the current week
 * It checks the limit for requests regarding the following ai-helper-tools: explanation, code-review, hints, commpiler
 */
var checkReqLimit = async function (req, res, next) {
  try {
    const userId = req.user.id;
    const userLimit = await requestSrv.getUserReqLimit(userId);

    if (userLimit && userLimit.limitExceeded) {
      // handle the case where the request limit has been exceeded
      return res.status(429).json(userLimit);
    }

    // if the user has not reached the request limit, continue processing the request
    next();
  } catch (err) {
    res.status(500).send(err);
  }
...
module.exports = {
  ...
  isValidCourseId,
  isProjectInCourse,
  isValidProjectIdAndSubmissionIdCombo,
  isValidProjectIdAndHelpRequestIdCombo,
  isValidProjectIdAndUserprojectIdCombo,
  isAdmin,
  isSelf,
  isOwner,
  isCourseOwner,
  isOwnerElseNextRoute,
  isCourseOwnerElseNextRoute,
  isUserElseNextRoute,
  isOwnerOrUserElseNextRoute,
  isPublicProject,
  setProjectIdFromLtiRequest,
  isLtiUserElseNextRoute,
  isLtiRequestAuthorized,
  isReturningLtiUserElseNextRoute,
  isLtiUserCreatableElseNextRoute,
  isLtiUserIdEqualsUserName,
  hasCourseId,
  checkReqLimit,
};
