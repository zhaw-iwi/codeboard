...
/**
 * This middleware function checks whether the current user is allowed to access full course data or not
 * @param req
 * @param res
 * @param next
 */
var _isCourseAdvisor = function(req, res, next, onFailUseNextRoute) {

  let courseId = req.params.courseId;
  let advisorRoles = ['owner', 'teacher'];

  db.CourseUserNew.findOne({
    where: {
      courseId: courseId,
      userId: req.user.id
    }
      ...
      .then(function(userCourse) {
        if(userCourse && advisorRoles.includes(userCourse.role)) {
          next();
        } else {
            if(onFailUseNextRoute) {
                next('route');
...
var isCourseAdvisor = function(req, res, next) {
  _isCourseAdvisor(req, res, next, false);
};

var isCourseAdvisorElseNextRoute = function(req, res, next) {
  _isCourseAdvisor(req, res, next, true);
};
...
exports.isCourseAdvisor = isCourseAdvisor;
exports.isCourseAdvisorElseNextRoute = isCourseAdvisorElseNextRoute;
