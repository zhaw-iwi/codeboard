                  ...
                  compilerErrorAI(payload.compilation)
                    .then((res) => {
                      ...
                      if (errorExplanation) {
                        ...
                        // fallback to default if user reached request limit
                        fallBackExpGeneration(payload);
                      } else {
        ...
        function ($rootScope, $scope, $uibModalInstance, $http, $timeout) {
          ...
          let enoughTestsPassed = async function () {
            ...
            if ($scope.isActionHidden('codeReview')) {
              ...
              $scope.textAfterResult =
                'Bitte warte einen Moment, während ich deinen Code reviewe und die Musterlösung lade.';
            }
            ...
            if (!$scope.isActionHidden('codeReview')) {
              try {
                ...
                if (res && res.answer) {
                  ...
                  $scope.textAfterResult = 'Du kannst dir nun das Code-Review ansehen.';

                  // save the code review in the backend
                  await ChatSrv.addChatLine(res.answer, null, 'Roby', 'codeReview');
                } else if (res && res.limitExceeded) {
                  // handle case when ai request limit is reached
                  $scope.reviewIsLoading = false;
                  $scope.codeReview =
                    'Es tut mir leid, aber ich kann dir momentan kein Code-Review geben. Du hast dein Anfrage-Limit erreicht.';
                }
              } catch (err) {
                console.log('Error while fetching code review!' + err);
                $scope.reviewIsLoading = false;
                $scope.textAfterResult =
                  'Es ist ein Fehler beim Laden des Code-Reviews aufgetreten. Bitte versuche es später erneut.';
              }
            }
...
app.controller('RightBarCtrl', [
  ...
  'TabService',
  'CodingAssistantCodeMatchSrv',
  function ($scope, $rootScope, ProjectFactory, IdeMsgService, TabService, CodingAssistantCodeMatchSrv) {
