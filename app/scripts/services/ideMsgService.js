...
angular.module('codeboardApp').service('IdeMsgService', function IdeMsgService() {
   ...
   */
  this.msgNavBarRightOpenTab = function (tab, doIoTest = false) {
    let payload = {
      tab: tab,
    };
    // only add to payload when we have to run the io test automatically (in test tab)
    if (doIoTest) {
      payload.doIoTest = doIoTest;
    }
